...
export const TagPage = (): React.ReactElement => {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    ...
    try {
      const [p, t] = await Promise.all([
        ...
        api.todosForTag(activeOrgId, name),
      ]);
      setProjects(p);
      setTodos(t);
    } catch (err) {
  ...
  return (
    <Stack>
      ...
      <List disablePadding>
        {todos.length === 0 && error === null && (
          <Typography color="text.secondary">No todos with this tag.</Typography>
        )}
        {todos.map((todo) => (
          <TodoItem
            key={todo.id}
